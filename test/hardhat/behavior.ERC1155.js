...
// **************************************
	...
	async function shouldRevertWhenInvalidReceiver(promise, contract, receiver, error) {
		if (typeof error === 'undefined') {
			await expect(promise)
				.to.be.revertedWithCustomError(contract, `IERC1155_INVALID_RECEIVER`)
				.withArgs(receiver)
		}
		else {
			if (error == ERC1155ReceiverError.RevertWithError) {
				await expect(promise)
					.to.be.revertedWithCustomError(receiver, `ERC1155ReceiverError`)
			}
		...
		describe(`Should behave like IERC1155 at deploy`, function () {
			...
			// **************************************
				describe(CONTRACT.METHODS.DEFAULT_SERIES.SIGNATURE, function () {
					it(`Should be Series ID number ${ TEST.DEFAULT_SERIES }`, async function () {
						const expected = TEST.DEFAULT_SERIES
						expect(
							await contract.DEFAULT_SERIES()
						).to.equal(expected)
					})
				})
				describe(CONTRACT.METHODS.exists.SIGNATURE, function () {
					it(`Should be the FALSE by default`, async function () {
						const id = TEST.DEFAULT_SERIES
						expect(
	...
	async function shouldBehaveLikeIERC1155AfterCreatingSeries(fixture, TEST, CONTRACT) {
		describe(`Should behave like IERC1155 after creating series`, function () {
			...
			// **************************************
				describe(CONTRACT.METHODS.exists.SIGNATURE, function () {
					it(`Should be the TRUE once series is set`, async function () {
			...
			// **************************************
				describe(CONTRACT.METHODS.safeBatchTransferFrom.SIGNATURE, function () {
					...
					it(`Should be reverted when transfering to the NULL address`, async function () {
						...
						const amounts = [TEST.TARGET_AMOUNT]
						const data = "0x"
						await shouldRevertWhenInvalidReceiver(
							contract
								.connect(from)
	...
	async function shouldBehaveLikeIERC1155MetadataURI(fixture, TEST, CONTRACT) {
		describe(`Should behave like IERC1155MetadataURI`, function () {
			beforeEach(async function () {
				const {
					test_contract,
				} = await loadFixture(fixture)
