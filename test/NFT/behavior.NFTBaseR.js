...
// *****        TEST  SUITES        *****
// **************************************
	function shouldBehaveLikeNFTBaseRAtDeploy ( fixture, TEST, CONTRACT  ) {
				...
				// **************************************
					describe( CONTRACT.METHODS.airdrop.SIGNATURE, function () {
						if ( TEST.METHODS.airdrop ) {
							it( `Airdrop tokens should emit a Transfer event`, async function () {
								const accounts = [
									users[ USER1 ].address,
									users[ USER2 ].address,
								]
	...
	function shouldBehaveLikeNFTBaseRAfterSettingStateToOpen ( fixture, TEST, CONTRACT ) {
				...
				// **************************************
					describe( CONTRACT.METHODS.mintPublic.SIGNATURE, function () {
						if ( TEST.METHODS.mintPublic ) {
							it( `Minting 1 token should emit a Transfer event`, async function() {
								const qty       = 1
								const fromAddr  = ethers.constants.AddressZero
							...
							it( `Minting 2 tokens should emit a Transfer event`, async function() {
								const qty       = 2
								const fromAddr  = ethers.constants.AddressZero
								const toAddr    = users[ TOKEN_OWNER ].address
								const fromToken = TEST.FIRST_TOKEN
							...
							it( `Minting ${ TEST.PARAMS.maxBatch_ } tokens should emit a Transfer event`, async function() {
